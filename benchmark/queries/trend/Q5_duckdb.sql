...
FROM (
    ...
    FROM (SELECT deviceId, (datetime((CAST(strftime('%s', ts) AS INTEGER) / (30 * 60)) * (30 * 60), 'unixepoch')) AS time_interval, AVG(HR) as interval_HR FROM hrm GROUP BY time_interval, deviceId) AS h
    JOIN 
