...
class BenchmarkConfig:
    ...
    def _validate_config(self) -> None:
        ...
        # Validate engines
        unsupported_engines = set(self._config["engines"]) - SUPPORTED_ENGINES
    ...
    def engines(self) -> List[str]:
        """Get list of database engines."""
        return self._config["engines"]
    ...
    @property
    def query_groups(self) -> List[Dict[str, Any]]:
        """Get list of query group configurations."""
        return self._config["query_groups"]
    ...
    def get_threads(self, engine: str) -> int:
        ...
        """
        thread_mapping = {
            "duckdb": self._config.get("threads_duckdb", 4),
            "chdb": self._config.get("threads_chdb", 4),
            "sqlite": self._config.get("threads_sqlite", 0),
        }
        return thread_mapping.get(engine, 0)
    
    @property
    def repeat_full(self) -> int:
        """Get number of repetitions for full run."""
        return self._config["repeat_full"]
    
    @property
    def warmups_full(self) -> int:
        """Get number of warmups for full run."""
        return self._config["warmups_full"]
    
    @property
    def repeat_pilot(self) -> int:
        """Get number of repetitions for pilot run."""
        return self._config["repeat_pilot"]
    
    @property
    def warmups_pilot(self) -> int:
        """Get number of warmups for pilot run."""
        return self._config["warmups_pilot"]
    
    @property
    def target_samples(self) -> int:
        """Get target number of samples."""
        return self._config["target_samples"]
    
    @property
    def min_interval(self) -> float:
        """Get minimum interval between runs."""
        return self._config["min_interval"]
    
    @property
    def max_interval(self) -> float:
        """Get maximum interval between runs."""
